...
<Link href="/makelists">
  <button className="bg-white p-3 rounded-md text-2xl w-fitcontent mx-auto dark:text-maroonx-11 border border-black">Make your list here!</button>
</Link>
</section>
    <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mx-16 my-10">
        {wishlists?.map(({ teacherName, location, bio, link, id }) => (
          <div className="flex flex-col bg-orange-grad rounded-xl py-4 px-8 text-center border border-black dark:text-black" key={id}>
            ...
            <a href={link} className="font-sans text-lg bg-white rounded w-1/4 mx-auto p-3 m-2 align-center text-maroonx-11 dark:bg-black dark:text-white">Link</a>
          </div>
